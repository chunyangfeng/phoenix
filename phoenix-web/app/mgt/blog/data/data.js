...
// 博客专栏table表格
const initialArticleSerialTable = ()=> {
    layui.table.render({
        elem: '#article-serial-table',
        url: urls.serialListApi,
        id: 'articleTagTable',
        page: true,
        limit: 15,
        limits: [15, 30, 45, 60, 100],
        ...
        cols: [[
            { fixed: 'left', title: '选中', align: "center", type: 'checkbox'},
            {field: 'id', title: 'ID', align: "center", sort: true, hide: true},
            {field: 'name', title: '专栏名称', align: "center"},
            {field: 'desc', title: '描述', align: "center"},
            {field: 'ctime', title: '创建时间', align: "center"},
            {field: 'mtime', title: '修改时间', align: "center"},
            {fixed: 'right', title: '操作', align: 'center', toolbar: `${params.rowActionElem}`}
...
// 博客专栏新增/编辑事件
const serialAddEvent = (obj, type='新增') => {
    layerIframe(urls.articleSerialPage, `${type}博客专栏`, 'serialIframe', ['30rem', '17rem']);
};

// 博客标签批量删除事件
const serialBulkDeleteEvent = (obj) => {
    tableBulkDelete(obj, urls.serialListApi);
};
...
// 博客专栏编辑事件
const serialEditEvent = (obj) => {
    assigmentAttribute(window, "serialFormFilter", obj.data);
    serialAddEvent('编辑');
};
...
layui.jquery(document).ready(function () {
    ...
    // 监听搜索表单提交事件
    layui.form.on('submit(articleSerialSearchForm)', function (data) {
        layTableReload('articleSerialTable', data.field);
        return false
    });
    ...
    layui.table.on(`toolbar(${params.classifyTableFilter})`, function (obj) {
        ...
        layTableReload('articleClassifyTable');
    });
    ...
    layui.table.on(`toolbar(${params.tagTableFilter})`, function (obj) {
        ...
        layTableReload('articleTagTable');
    });
    ...
    // 监听表格头部工具事件
    layui.table.on('toolbar(articleSerialTableFilter)', function (obj) {
        tableToolbarEventHandle(obj, serialAddEvent, serialBulkDeleteEvent);
        layTableReload('articleSerialTable');
    });

    // 监听表格行工具事件
    layui.table.on('tool(articleSerialTableFilter)', function (obj) {
        tableRowEventHandle(obj, urls.serialInfoApi, null, serialEditEvent);
        layTableReload('articleSerialTable');
    });
});
